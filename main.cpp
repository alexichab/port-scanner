...
int main(int argc, char *argv[]) {
  ...
  if (start_port > end_port || num_threads <= 0) {
    std::cerr << "Invalid arguments: start_port must be <= end_port, "
  ...
  if (num_threads > total_ports)
    num_threads = total_ports; // Оптимизация потоков
  ...
  std::thread progress_thread([&]() {
    ...
    while (!progress_done) {
      ...
      if (percent != last_percent) {
        std::cout << "\rScanning: " << percent << "% (" << scanned_ports << "/"
                  << total_ports << ")   " << std::flush;
    ...
    std::cout << "\rScanning: 100% (" << scanned_ports << "/" << total_ports
              << ")   " << std::endl;
  });
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    if (i < extra_ports)
      thread_end++;
    if (thread_end > end_port)
      thread_end = end_port;
    current_port = thread_end + 1;

    auto fut = std::async(std::launch::async, scan_ports_progress, ip,
